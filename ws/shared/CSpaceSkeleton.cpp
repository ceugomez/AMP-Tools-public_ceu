...
std::unique_ptr<amp::GridCSpace2D> MyManipulatorCSConstructor::construct(const amp::LinkManipulator2D& manipulator, const amp::Environment2D& env) {
    ...
    for (int i = 0; i < m_cells_per_dim; ++i) {              // for each element in dimension 1
        for (int j = 0; j < m_cells_per_dim; ++j) {          // for each element in dimension 2
            ...
            cspace_state[0] = i * (2 * pi / m_cells_per_dim); // set joint angle, evenly spaced between 0 and 2pi, for x1
            cspace_state[1] = j * (2 * pi / m_cells_per_dim); // set joint angle, evenly spaced between 0 and 2pi, for x2
